...
pub struct WindowsWindowState {
    ...
    /// Inside a modal `WM_ENTERSIZEMOVE` loop (native titlebar drag or live
    /// resize). `size_move_resized` records whether a `WM_SIZE` has arrived —
    /// a pure move leaves the scene identical, so `draw_window` skips
    /// rendering instead of stalling the modal message pump.
    pub size_move_loop: Cell<bool>,
    pub size_move_resized: Cell<bool>,
...
impl WindowsWindowState {
    fn new(
        ...
        Ok(Self {
            ...
            size_move_loop: Cell::new(false),
            size_move_resized: Cell::new(false),
...
pub(crate) struct Callbacks {
    ...
    pub(crate) hit_test_window_control_position: Cell<Option<Point<Pixels>>>,
...
impl PlatformWindow for WindowsWindow {
    ...
    fn hit_test_window_control_position(&self) -> Option<Point<Pixels>> {
        self.0
            .state
            .callbacks
