...
impl WindowsWindowInner {
    pub(crate) fn handle_msg(
        ...
        let handled = match msg {
            ...
            WM_ENTERSIZEMOVE => self.handle_size_move_loop(handle, true),
            WM_ENTERMENULOOP => self.handle_size_move_loop(handle, false),
    ...
    fn handle_size_msg(self: &Rc<Self>, wparam: WPARAM, lparam: LPARAM) -> Option<isize> {
        // A WM_SIZE inside the modal loop means this is a resize, not a pure
        // move — live-resize still needs painting.
        if self.state.size_move_loop.get() {
            self.state.size_move_resized.set(true);
        }
    ...
    fn handle_size_move_loop(&self, handle: HWND, is_size_move: bool) -> Option<isize> {
        self.state.size_move_loop.set(is_size_move);
        self.state.size_move_resized.set(false);
        unsafe {
            let ret = SetTimer(
                Some(handle),
    ...
    fn handle_size_move_loop_exit(&self, handle: HWND) -> Option<isize> {
        self.state.size_move_loop.set(false);
        self.state.size_move_resized.set(false);
        unsafe {
    ...
    fn handle_mouse_leave_msg(&self, handle: HWND) -> Option<isize> {
        ...
        // Windows does not report a final client mouse move when the cursor
        // leaves the client area (into the non-client area or out of the
        // window). Without a MouseExited the element under the last cursor
        // position keeps its hovered state forever — a visibly "stuck" hover.
    ...
    fn handle_hit_test_msg(&self, handle: HWND, lparam: LPARAM) -> Option<isize> {
        ...
        // Cursor position carried by this WM_NCHITTEST: screen -> client
        // coords, then logical pixels for control-area hit testing.
        let mut cursor_point = POINT {
            x: lparam.signed_loword().into(),
            y: lparam.signed_hiword().into(),
        };
        ...
        let drag_area = if let Some(mut callback) = callback {
            self.state
                .callbacks
                .hit_test_window_control_position
                .set(Some(position));
            let area = callback();
            self.state
                .callbacks
    ...
    fn draw_window(&self, handle: HWND, force_render: bool) -> Option<isize> {
        // Inside the modal size/move loop a pure position change leaves the
        // scene identical — DWM just translates the composited surface.
        // Rendering anyway stalls the pump for a whole frame and makes the
        // drag judder. A WM_SIZE in the loop means live-resize, which still
        // paints normally.
        if !force_render
            && self.state.size_move_loop.get()
            && !self.state.size_move_resized.get()
        {
