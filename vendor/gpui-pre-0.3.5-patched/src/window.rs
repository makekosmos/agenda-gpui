                ...
                handle
                    .update(&mut cx, |_, window, _cx| {
                        let position = window
                            ...
                            .unwrap_or(window.mouse_position);
                        let hit_test = window.rendered_frame.hit_test(position);
...
impl Window {
    ...
    pub fn bounds_changed(&mut self, cx: &mut App) {
        ...
        // A pure position change (every WM_MOVE while the window is dragged)
        // leaves the rendered scene identical; repainting then just stalls the
        // modal size/move loop. Only dirty the window when the output changed.
        let output_changed = scale_factor != self.scale_factor
    ...
    pub fn dispatch_event(&mut self, event: PlatformInput, cx: &mut App) -> DispatchEventResult {
        ...
        let event = match event {
            ...
            PlatformInput::MouseExited(mouse_exited) => {
                // Record the exit position so the per-frame `mouse_hit_test`
                // recomputation does not resurrect hover on whatever element
                // sits at the stale pre-exit position.
