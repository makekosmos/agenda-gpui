                ...
                let update_hover = move |is_hovered: bool, window: &mut Window, cx: &mut App| {
                    ...
                    if is_hovered != *was_hovered {
                        *was_hovered = is_hovered;
                        ...
                        // Unlike the internal hover_style listener above,
                        // on_hover callbacks are user code: they mutate state
                        // but nothing schedules a repaint, so elements without
                        // a hover_style (active items, plain rows) would keep
                        // painting a stale hover.
                        cx.notify(current_view);
                    }
                };
