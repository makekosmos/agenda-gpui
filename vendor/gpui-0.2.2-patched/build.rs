...
mod windows {
    ...
    pub(super) fn build() {
        ...
        // Embed the Windows manifest and resource file (native Windows hosts only;
        // embed-resource is a target_os=windows build-dep and unavailable when
        // cross-compiling from Linux — agenda-gpui embeds its own app.manifest).
